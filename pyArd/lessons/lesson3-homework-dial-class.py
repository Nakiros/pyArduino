...
class MultiDial:
    def __init__(self,dialTitle,minVal,maxVal, unitOfMeasure, noOfMTick,noOfmTick,coloR,coloG,coloB, posX,posY,posZ):
        ...
        bgBox=box(pos=vector(self.posX,(bgHeight/2)+self.posY,(-bgThick/2)+self.posZ), size=vector(bgLength,bgHeight,bgThick))
        ...
        indicaTOR= arrow(axis=vector(-1, 0, 0), color=majorTick.color,
                   shaftwidth=majorTickT, length=arrowL, pos=vector(self.posX, self.posY, .5+self.posZ))
        valueLabel=label(text=str(self.incomingValue), color=majorTick.color,height=24, border=4, pos=vector(
