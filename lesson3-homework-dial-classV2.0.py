...
while True:
    ...
    lenSplitPacket=len(splitPacket)
    print(splitPacket)
    
    #UPDATE arrows and lables for all dials
    for i in range(0, NoOfDials, 1):
        constrainedValue=constrainValue(float(splitPacket[i]),dialsList[i].minVal,dialsList[i].maxVal)
        ...
        if (float(splitPacket[i])>=dialsList[i].minVal) and (float(splitPacket[i])<=dialsList[i].maxVal):
            dialsList[i].bgBox.color=color.white
        else:
            dialsList[i].bgBox.color=color.red
            print(dialsList[i].dialTitle,'is out of bounds.')
    
    #TODO check the code below for
    """
    if NoOfDials < lenSplitPacket:
        for i in range(NoOfDials,lenSplitPacket,1):
            if i!= (lenSplitPacket-1):
                print(splitPacket[i], end='    ')
            else:
                print(splitPacket[i])
    
    if NoOfDials>lenSplitPacket:
        for i in range(lenSplitPacket,NoOfDials,1):
            #update axis od dial i to go in the middle and change the background to grey
            dialsList[i].indicaTOR.axis=vector(arrowL*np.cos(.5*np.pi),arrowL*np.sin(.5*np.pi),0)
            dialsList[i].bgBox.color=vector(.5,.5,.5)
    """